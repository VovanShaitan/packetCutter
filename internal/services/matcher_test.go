...
func TestMatcherService_CountMatchesForSequence(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			name:          "valid sequence",
			target:        "123",
			sequence:      domain.Sequence{"1", "2", "3"},
			expected:      3,
			expectedError: nil,
		},
		{
			...
			target:        "123",
			sequence:      domain.Sequence{"1", "2"},
			expected:      0,
			expectedError: domain.ErrSequenceLength,
		},
		{
			name:          "multi char sequence",
			target:        "123",
			sequence:      domain.Sequence{"12", "23", "34"},
			expected:      3,
			expectedError: nil,
		},
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			count, err := matcher.CountMatchesForSequence(tt.target, tt.sequence)
